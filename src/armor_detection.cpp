...
//        finalArmor = Armor();
    ...
    else if(candidateArmors.size() == 1)
    {
        ...
        Mat out_blobs = dnnDetect.net_forward(numBlobs);
...
//            return candidate1.size.height > candidate2.size.height;});
    ...
    {
        int max_grade = finalArmors[0].grade;
        for(auto & i : finalArmors)
        {
        ...
        for (int j = 0; j < 4; j++)
        {
            line(showSrc, finalArmor.armor_pt4[j], finalArmor.armor_pt4[(j + 1) % 4], CV_RGB(255, 255, 0), 2);
        }

        double ff = finalArmor.grade;
        string information = to_string(finalArmor.id) + ":" + to_string(finalArmor.confidence*100) + "%";
        //        putText(final_armors_src,ff,finalArmors[i].center,FONT_HERSHEY_COMPLEX, 1.0, Scalar(12, 23, 200), 1, 8);
        putText(showSrc, information,finalArmor.armor_pt4[3],FONT_HERSHEY_COMPLEX,0.5,Scalar(255,0,255));
        imshow("showSrc", showSrc);
    }
...
vector<Armor> ArmorDetector::autoAim(const cv::Mat &src)
{
    ...
    if(!numBlobs.empty())numBlobs.clear();
...
void ArmorDetector::preImplement(Armor& armor)
{
    ...
    dnnDetect.img_processing(numDst, numROIs, numBlobs);
}
...
bool ArmorDetector::get_valid(const float *data, float &confidence, int &id)
{
    ...
    if(data[0] - data[1]>-0.9 || id == 2 || confidence < thresh_confidence)
        return false;
    else
        return true;
}
